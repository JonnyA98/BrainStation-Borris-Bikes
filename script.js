...
axios.get(BASE_URL_FOR_WEATHER).then((response) => {
  ...
  const howsTheWeather = () => {
    ...
    if (wind <= 25 && rain <= 2 && 16 <= temp && temp < 25) {
      borisPic.src = "./assets/Boris-Pics/Marvelous-Boris.jpeg";
...
const getBikes = () => {
  axios.get(BASE_URL_FOR_BIKES).then((response) => {
    ...
    if (highstreet >= 10) {
      ...
      bikeText.classList.add("bike__text--green");
      ...
      bikeText.classList.add("bike__text--amber");
    } else {
      bikeText.innerText = `TOO SLOW! There are no Bikes left! Instead you can go to Worship Street (${worship} bikes left) or Columbia Street (${columbia} bikes left)`;
    }
  });
};
